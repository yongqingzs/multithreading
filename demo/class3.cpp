...
// void myprint01(const int i, string& pmybuf)
{
    cout << i << endl;
...
public:
    ...
    A(int a) :m_i(a)
    {
        ...
        cout << "[A::A(int a) constructor executed]" << this << " thread = " << std::this_thread::get_id() << endl;
    }
    A(const A &a) :m_i(a.m_i)
    {
        //cout << "[A::A(int a)拷贝构造函数执行]" << this << endl;
        cout << "[A::A(int a) copy constructor executed]" << this << " thread = " << std::this_thread::get_id() << endl;
    }
    ~A()
    {
        // cout << "[A::A() destructor executed]" << this << endl;
        cout << "[A::A() destructor executed]" << this << " thread = " << std::this_thread::get_id() << endl;
    }

    void thread_work(int num)
        //来个参数
    {
        cout << "[Child thread thread_work executed]" << this << " thread = " << std::this_thread::get_id() << endl;
    }

    void operator() (int num)
    {
        cout << "[Child thread () executed]" << this << " thread = " << std::this_thread::get_id() << endl;
    }
};

void myprint02(const int i, const A &pmybuf)
{
    cout << "myprint02: " << &pmybuf << endl;
    //这里打印的是pmybuf对象的地址
    return;
}
...
{
    cout << "Child thread myprint2 parameter address is: " << &pmybuf << " thread = " << std::this_thread::get_id() << endl;
...
int main04()
{
    ...
    //mytobj将类对象作为线程参数
    // std::thread mytobj(myprint03, std::ref(myobj));
    std::thread mytobj(myprint03, myobj);
    // std::thread mytobj(myprint04, myobj);//error
...
{
	cout << "Child thread myprint2 parameter address is: " << &pzn << " thread = " << std::this_thread::get_id() << endl;
    return;
}
...
int main05()
{
    ...
    std::thread mytobj(myprint05, std::ref(myp));
    // std::thread mytobj(myprint05, myp);
...
int main06()
{
    A myobj(10);
    //生成一个类对象
    // std::thread mytobj(&A::thread_work, myobj, 15);  //A::thread_work不行, myobj.thread_work不行
    //join和detach都可以 
