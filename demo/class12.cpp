...
public:
    ...
    void inMsgRecvQueue()
    {
        ...
        {
            cout << "inMsgRecvQueue() executing, inserting an element " << i << endl;
    ...
    void outMsgRecvQueue()
    {
        ...
        for (int i = 0; i < 100000; ++i)
        {
            ...
            if (result == true)
            {
                cout << "outMsgRecvQueue() executing, taking out an element " << command << endl;
                //这里可以考虑处理数据
                //.....
            }
            else
            {
                cout << "outMsgEecvQueue() executing, but the message queue is currently empty " << i << endl;
            }
        }
        cout << "end" << endl;
    }
...
int main04()
{
    AA myobja;
#if 1
    // outMsgRecvQueue不上线程锁时，触发线程冲突错误的概率不大，但还是会出现
    std::vector<std::thread> myThreads;
    myThreads.push_back(std::thread(&AA::inMsgRecvQueue, &myobja));
    for (int i = 0; i < 5; i++)
        myThreads.push_back(std::thread(&AA::outMsgRecvQueue, &myobja));
