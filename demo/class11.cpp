...
int main01()
{
    ...
    mytobj1.join();
    mytobj2.join();
    cout << "Both threads have finished, the final result of g_mycout is " << g_mycout << endl;
...
int mythread2()
{
	...
	std::chrono::milliseconds dura(1000);	//5秒
...
int main02()
{
	...
	cout << "I am before result.get()" << endl;
	cout << "I am before result.get()" << endl;
	cout << "I am before result.get()" << endl;
	cout << "I am before result.get()" << endl;
	cout << "I am before result.get()" << endl;
...
int mythread3()
{
    ...
    std::chrono::milliseconds dura(1000);	//1秒 = 1000毫秒，所以5000毫秒 = 5秒
...
// int main03()
{
    ...
    std::future_status status = result.wait_for(std::chrono::milliseconds(1001));	
    if (status == std::future_status::deferred)
    {
        //线程被延迟执行了（系统资源紧张了，它给我采用std::launch::deferred策略了）
		cout << "Thread is deferred!" << endl;
    ...
    else
    {
        ...
        {
            //线程成功返回
            cout << "Thread executed successfully and returned!" << endl;
